...
class S3Plugin {
  constructor () {
    ...
    this.routes = [
      { verb: 'get', url: '/getUploadUrl', controller: 'fileController', action: 'getUploadUrl' },
      { verb: 'get', url: '/files/:fileKey', controller: 'fileController', action: 'getFileUrl' },
      { verb: 'put', url: '/files/:fileKey', controller: 'fileController', action: 'validateUploadedFile' },
      { verb: 'delete', url: '/files/:fileKey', controller: 'fileController', action: 'deleteFile' }
    ];
  }
   ...
   * the "validateUploadedFile" route within the configured TTL 
  ...
  async getUploadUrl (request) {
    ...
    const 
      ...
      fileKey = `${this.config.uploadDir}/${uuid()}/${filename}`;
    ...
    return {
      fileKey,
      uploadUrl,
      fileUrl: this._getFileUrl(fileKey)
    };
  }
   ...
   * @param {Request} request 
   */
  async validateUploadedFile (request) {
    this._assertCredentials();
    ...
    await this.sdk.ms.del([redisKey]);
  ...
  async getFileUrl (request) {
    ...
    return this._getFileUrl(fileKey);
  }
