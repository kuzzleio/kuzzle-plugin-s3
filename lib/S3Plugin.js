...
class S3Plugin {
  ...
  async uploadGetUrl(request) {
    ...
    this._expireFile(Bucket, fileKey);
  ...
  async bucketCreate (request) {
    ...
    if (!this._validateBucketName(bucketName, disableDotsInName)) {
      throw new this.context.errors.BadRequestError(`Bucket name ${bucketName} contains unauthorized character(s)`);
    }
