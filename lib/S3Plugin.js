...
class S3Plugin {
  constructor() {
    ...
    this.controllers = {
      ...
      bucket: {
        ...
        setBucketPolicy: req => this.setBucketPolicy(req),
    ...
    this.routes = [
      ...
      {
        verb: 'post',
        url: '/bucket/publicAccess/:bucketName/:bucketRegion',
        controller: 'bucket',
        action: 'enablePublicAccess'
      },
      {
        verb: 'post',
        url: '/bucket/bucketPolicy/:bucketName/:bucketRegion',
        controller: 'bucket',
  ...
  async bucketCreate(request) {
    ...
    try {
      createBucketResponse = await this.s3.createBucket({ ...params, ...bucketOptions }).promise();
    ...
    // minio does not support CORS & don't need to: https://github.com/minio/minio/issues/15874
    ...
    return {
      ...
      CORS: bucketCORS,
      createBucketOutput: createBucketResponse.$response.data
    };
  }

  /**
   * Set bucket policy in bucket
   *
   * @controller            bucket
   * @action                setBucketPolicy
   *
  ...
  async setBucketPolicy(request) {
    ...
    const bucketPolicy = request.input.body.bucketPolicy;

    if (!bucketPolicy) {
      throw new this.context.errors.BadRequestError(
        '[S3Plugin.setBucketPolicy] missing "bucketPolicy" param'
      );
    }

    let bucketPolicyJson;
    try {
      bucketPolicyJson = JSON.stringify(bucketPolicy);
    }catch (bucketPolicySerializationError) {
      throw new this.context.errors.BadRequestError(
        `[S3Plugin.setBucketPolicy] bucket policy is not valid JSON "${bucketPolicy}"`
      );
    }

    AWS.config.update({ region: bucketRegion });

    // minio support bucketPolicies too, no need to skip:
    ...
    await this.s3.putBucketPolicy({ Bucket: bucketName, Policy: bucketPolicyJson }).promise();
  ...
  async bucketExists(request) {
    ...
    try {
      ...
      // S3 is NotFound, Minio is InvalidAccessKeyId
      if (err.code === 'NotFound' || err.code === 'InvalidAccessKeyId') {
        return false;
      }
      

      // let client handle error
      throw err;
    }
  }
