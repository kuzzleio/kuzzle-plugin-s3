...
class S3Plugin {
  constructor() {
    ...
    this.controllers = {
      ...
      file: {
        ...
        getFilesKeys: req => this.getFilesKeys(req)
      },
      bucket: {
  ...
  async uploadGetUrl(request) {
    ...
    const filename = this._stringArg(request, 'filename'),
      ...
      Bucket = this._stringArg(request, 'bucketName', this.config.bucketName),
    ...
    return {
      fileKey,
      uploadUrl,
      fileUrl: this._getUrl(Bucket, fileKey),
  ...
  async getFilesKeys(request) {
    ...
    const Bucket = this._stringArg(request, 'bucketName', this.config.bucketName);
   ...
   */
  async _fileExists(Bucket, fileKey) {
    try {
      await this.s3
        .headObject({
          Bucket,
   ...
   */
  async fileGetUrl (request, signedUrl = false) {
    this._assertCredentials()
    const
   ...
   */
    _validateBucketName (name, disableDotsInName=false) {
      const bucketNameRegex = disableDotsInName ? 
        ...
        const sanitizedName = bucketNameRegex.test(name)
  
      if (sanitizedName === null) {
        throw new this.context.errors.BadRequestError(`Invalid bucket name format: ${name}`)
      }
    }
  ...
  async bucketCreate (request) {
    ...
    const
      bucketName = this.stringArg(request, 'bucketName', this.config.bucketName),
      bucketRegion = this.stringArg(request, 'bucketRegion', this.config.region),
    ...
    if (this.bucketExists(request)){
      // TODO: add 409 conflict error template to kuzzle ?
      throw new this.context.errors.BadRequestError(`Bucket name already exist ${err.message}`);
    }

    if (!_validateBucketName(bucketName, disableDotsInName)){
      throw new this.context.errors.BadRequestError(`Bucket name ${bucketName} contains unauthorized character(s) ${err.message}`);
    }
  ...
  async bucketDelete (request) {
    ...
    const bucketRegion = this.stringArg(request, 'bucketRegion', this.config.bucketRegion);
