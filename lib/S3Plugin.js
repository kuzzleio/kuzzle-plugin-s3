...
//https://stackoverflow.com/a/54341763/3744415
...
class S3Plugin {
  ...
  async getFilesKeys () {
    this._assertCredentials();
    const s3KeyList = await listAllObjects(this.s3, {Bucket: this.config.bucketName});
    const kuzzleKeyList = s3KeyList.map(k => ({
      ...k,
      Key: this._getUrl(k.Key)
      }));
    return {
      filesKeys: kuzzleKeyList
    };
  }
