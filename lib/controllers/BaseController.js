...
class BaseController {
  ...
  getS3Client(region) {
    ...
    const client = getS3Client({
      ...
      region,
      forcePathStyle: endpointConfig.forcePathStyle || this.config.isMinio || this.config.forcePathStyle || false,
    });
  ...
  _getCredentials(targetRegion) {
    ...
    const accessKeyId =
      ...
      getProperty(this.context.secrets, endpointConfig.accessKeyIdPath);

    const secretAccessKey =
      process.env.AWS_SECRET_ACCESS_KEY ||
      getProperty(this.context.secrets, endpointConfig.secretAccessKeyPath);

    if (!accessKeyId || !secretAccessKey) {
      throw new this.context.errors.InternalError(
        `S3 credentials are missing for region: ${targetRegion}. Ensure they are set in the vault or environment variables.`
      );
    }
 ...
 */
  _getUrl(bucketName, fileKey, region) {
    ...
    const forcePathStyle = endpointConfig.forcePathStyle || this.config.forcePathStyle || false;
