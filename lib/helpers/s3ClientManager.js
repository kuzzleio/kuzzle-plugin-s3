 ...
 * @param {Object} [options.s3ClientOptions={}] - Additional options to pass to the S3 client.
...
function getS3Client(
  { endpoint, region, forcePathStyle = false, s3ClientOptions = {} },
  credentials
) {
  ...
  if (!clients[clientKey]) {
    if (
      ...
      throw new Error(
        `Credentials (accessKeyId and secretAccessKey) are required for region ${region} and endpoint ${endpoint}.`
      );
    }
...
async function listAllObjects(s3, params) {
  ...
  while (isTruncated) {
    ...
    if (objects.length > 0) {
      files.push(...listResponse.Contents);
    }
